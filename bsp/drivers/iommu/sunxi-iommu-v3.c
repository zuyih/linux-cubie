...
#define IOMMU_INT_ERR_LOW_ADDR0_REG		0x0110
#define IOMMU_INT_ERR_HIGH_ADDR0_REG		0x0114
#define IOMMU_INT_ERR_LOW_ADDR1_REG		0x0118
#define IOMMU_INT_ERR_HIGH_ADDR1_REG		0x011C
#define IOMMU_INT_ERR_LOW_ADDR2_REG		0x0120
#define IOMMU_INT_ERR_HIGH_ADDR2_REG		0x0124
#define IOMMU_INT_ERR_LOW_ADDR3_REG		0x0128
#define IOMMU_INT_ERR_HIGH_ADDR3_REG		0x012C
#define IOMMU_INT_ERR_LOW_ADDR4_REG		0x0130
#define IOMMU_INT_ERR_HIGH_ADDR4_REG		0x0134
#define IOMMU_INT_ERR_LOW_ADDR5_REG		0x0138
#define IOMMU_INT_ERR_HIGH_ADDR5_REG		0x013C
#define IOMMU_INT_ERR_LOW_ADDR6_REG		0x0140
#define IOMMU_INT_ERR_HIGH_ADDR6_REG		0x0144
#define IOMMU_INT_ERR_LOW_ADDR7_REG		0x0148
#define IOMMU_INT_ERR_HIGH_ADDR7_REG		0x014C
#define IOMMU_INT_ERR_LOW_ADDR8_REG		0x0150
#define IOMMU_INT_ERR_HIGH_ADDR8_REG		0x0154

#define IOMMU_INT_ERR_DATA0_REG			0x0160
#define IOMMU_INT_ERR_DATA1_REG			0x0164
#define IOMMU_INT_ERR_DATA2_REG			0x0168
#define IOMMU_INT_ERR_DATA3_REG			0x016C
#define IOMMU_INT_ERR_DATA4_REG			0x0170
#define IOMMU_INT_ERR_DATA5_REG			0x0174
#define IOMMU_INT_ERR_DATA6_REG			0x0178
#define IOMMU_INT_ERR_DATA7_REG			0x017C
#define IOMMU_INT_ERR_DATA8_REG			0x0180

#define IOMMU_L1PG_INT_REG			0x01B0
#define IOMMU_L2PG_INT_REG			0x01B4
#define IOMMU_LOW_VA_REG			0x01C0
#define IOMMU_HIGH_VA_REG			0x01C4
#define IOMMU_VA_DATA_REG			0x01C8
#define IOMMU_VA_CONFIG_REG			0x01CC
 ...
 *		#iommu-cells = <1>;
 *		version=<0x17>;            //IOMMU_VERSION_REG
 *		tlb_prefetch = <0x3007f>;  //IOMMU_TLB_PREFETCH_REG
 *		tlb_invalid_mode = <0x1>;  //IOMMU_TLB_IVLD_MODE_SEL_REG
 *		ptw_invalid_mode = <0x1>;  //IOMMU_PC_IVLD_MODE_SEL_REG
 *		masters = "ISP", "CSI", "VE0", "RESERVED", "G2D",
 *			"DE", "E907", "DEBUG_MODE";                //master id
 *		valid_masters=<7>;                                 //master num
 *		status = "enabled";
 *	};
 *
 *	de@xxxxx {
...
struct sunxi_iommu_plat_data {
	...
	char *master[IOMMU_MIC_MAX_MASTER];
};
...
static int sunxi_iommu_hw_init(struct iommu_domain *input_domain)
{
	...
	sunxi_iommu_write(iommu, IOMMU_TTB_REG, dte_addr >> 14);
...
int sunxi_iova_test_write(dma_addr_t iova, u32 val)
{
	...
	sunxi_iommu_write(iommu, IOMMU_LOW_VA_REG, iova & MASK_32_BIT);
	sunxi_iommu_write(iommu, IOMMU_HIGH_VA_REG, (iova >> 32) & MASK_32_BIT);
...
static irqreturn_t sunxi_iommu_irq(int irq, void *dev_id)
{
	...
	if (inter_status_reg & MICRO_TLB0_INVALID_INTER_MASK) {
		...
		addr_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_LOW_ADDR0_REG);
		addr_reg |= (u64)sunxi_iommu_read(iommu, IOMMU_INT_ERR_HIGH_ADDR0_REG) << 32;
		data_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_DATA0_REG);
	} else if (inter_status_reg & MICRO_TLB1_INVALID_INTER_MASK) {
		pr_err("%s Invalid Authority\n", plat_data->master[1]);
		addr_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_LOW_ADDR1_REG);
		addr_reg |= (u64)sunxi_iommu_read(iommu, IOMMU_INT_ERR_HIGH_ADDR1_REG) << 32;
		data_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_DATA1_REG);
	} else if (inter_status_reg & MICRO_TLB2_INVALID_INTER_MASK) {
		pr_err("%s Invalid Authority\n", plat_data->master[2]);
		addr_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_LOW_ADDR2_REG);
		addr_reg |= (u64)sunxi_iommu_read(iommu, IOMMU_INT_ERR_HIGH_ADDR2_REG) << 32;
		data_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_DATA2_REG);
	} else if (inter_status_reg & MICRO_TLB3_INVALID_INTER_MASK) {
		pr_err("%s Invalid Authority\n", plat_data->master[3]);
		addr_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_LOW_ADDR3_REG);
		addr_reg |= (u64)sunxi_iommu_read(iommu, IOMMU_INT_ERR_HIGH_ADDR3_REG) << 32;
		data_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_DATA3_REG);
	} else if (inter_status_reg & MICRO_TLB4_INVALID_INTER_MASK) {
		pr_err("%s Invalid Authority\n", plat_data->master[4]);
		addr_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_LOW_ADDR4_REG);
		addr_reg |= (u64)sunxi_iommu_read(iommu, IOMMU_INT_ERR_HIGH_ADDR4_REG) << 32;
		data_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_DATA4_REG);
	} else if (inter_status_reg & MICRO_TLB5_INVALID_INTER_MASK) {
		pr_err("%s Invalid Authority\n", plat_data->master[5]);
		addr_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_LOW_ADDR5_REG);
		addr_reg |= (u64)sunxi_iommu_read(iommu, IOMMU_INT_ERR_HIGH_ADDR5_REG) << 32;
		data_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_DATA5_REG);
	} else if (inter_status_reg & MICRO_TLB6_INVALID_INTER_MASK) {
		pr_err("%s Invalid Authority\n", plat_data->master[6]);
		addr_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_LOW_ADDR6_REG);
		addr_reg |= (u64)sunxi_iommu_read(iommu, IOMMU_INT_ERR_HIGH_ADDR6_REG) << 32;
		data_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_DATA6_REG);
		...
		addr_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_LOW_ADDR7_REG);
		data_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_DATA7_REG);
		addr_reg |= (u64)sunxi_iommu_read(iommu, IOMMU_INT_ERR_HIGH_ADDR7_REG) << 32;
	} else if (inter_status_reg & L2_PAGETABLE_INVALID_INTER_MASK) {
		if (!(int_masterid_bitmap & (1U << 31)))
			pr_err("L2 PageTable Invalid\n");
		addr_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_LOW_ADDR8_REG);
		addr_reg |= (u64)sunxi_iommu_read(iommu, IOMMU_INT_ERR_HIGH_ADDR8_REG) << 32;
		data_reg = sunxi_iommu_read(iommu, IOMMU_INT_ERR_DATA8_REG);
	} else
...
static int __init_plat_data_from_ofnode(struct device *dev, struct sunxi_iommu_plat_data *data,
					struct device_node *node)
{
	...
	if (count <= 0)
		return -ENODATA;
	for (i = 0; i < IOMMU_MIC_MAX_MASTER && i < count; i++) {
		data->master[i] = devm_kzalloc(dev, 16, GFP_KERNEL);
		if (!data->master[i])
			return -ENOMEM;
...
static int sunxi_iommu_probe(struct platform_device *pdev)
{
	...
	ret = __init_plat_data_from_ofnode(dev, sunxi_iommu->plat_data,
		dev->of_node);
	if (ret) {
		goto err_clk;
	}
